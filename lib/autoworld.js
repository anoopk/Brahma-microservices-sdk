...
exports.organisationCount = async (event, context, callback) => {
	...
	try{
		...
		await results.forEach(function(doc){
			organisation[doc.metadata.organization] = 0;
		});
		client.close();						
		return organisation;
	}	
...
exports.productCount = async (event, context, callback) => {
	...
	try{
		...
		let results = await coll.find();
		await results.forEach(function(doc){
			if(product[doc.metadata.product])
				product[doc.metadata.product] = product[doc.metadata.product]+1;
			else{
				product[doc.metadata.product] = 0;
			}
		});
		client.close();						
		return product;
